...
public class ElasticsearchUtilDeprecated {

	private static final Logger logger = LoggerFactory.getLogger(ElasticsearchUtilDeprecated.class);
	...
	public void writeEventsToElasticsearchIndex(String eventName, String destinationKey, String key){
		if (ES_EVENT_INDEX_ENABLED){
			...
			String json = new PayloadUtilDeprecated().createJSONWithEventInfo(eventName, destinationKey, key);
	...
	private void writeToElasticsearchIndex(String payload, String index, String type){
		...
		logger.debug("ElasticsearchUtil::writeToIndex::writing to index: " + index);
	...
	private void writeInboundMasterToElasticsearchIndex(String payload, String index, String type){
		...
		String id = new PayloadUtilDeprecated().getMasterIndexId(payload);
		String version = new PayloadUtilDeprecated().getMasterIndexVersion(payload);
		String versionType = new PayloadUtilDeprecated().getVersionType();
		...
		try{
			...
			logger.error("ElasticsearchUtil::writeToMasterIndex::FAILED writing to index " + index + " + " + payload);
			logger.error("ElasticsearchUtil::writeToMasterIndex::EXCEPTION: " + e.getMessage(), e);
		}
