...
public class PayloadUtilDeprecated {
	private static final Logger logger = LoggerFactory.getLogger(PayloadUtilDeprecated.class);
...
//	}	
	...
	public String getIndexNamePrefixForCustCustomerType(String json){
		...
		String custCustomerType = getCustCustomerType(json);
		String lCustCustomerType = custCustomerType.toLowerCase();
	...
	*/
	private String timestamp2microsec (String timestamp){
	...
	public String getMasterIndexVersion(String json){
		String sentTimeStamp = getSentTimestamp(json);
		
		String version = timestamp2microsec(sentTimeStamp);
	...
	public String getOrderDate(String json) {
		JSONObject jsonObject = new JSONObject(json);
		return jsonObject.getJSONObject(KEY_ORDER_HEADER).getString(KEY_ORDERDATE).trim();
	}
